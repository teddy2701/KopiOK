        ...
        {products.map(prod => (
            <div key={prod._id} className="bg-amber-50 rounded-lg p-4 border border-amber-200 hover:border-amber-300 transition-colors">
                <div className="space-y-2">
                    ...
                    {/* Tambahkan bagian resep di sini */}
                    <div className="mt-2">
                        <h4 className="text-sm font-medium text-amber-700 mb-2">Resep:</h4>
                        <ul className="space-y-1">
                            {prod.recipe.map((r, i) => (
                                <li key={i} className="text-sm text-amber-600 flex justify-between">
                                    <span>{r.material.name}</span>
                                    <span>
                                        {r.amountPerUnit} 
                                        <span className="ml-1 text-amber-500">
                                            {formatAmount(r.material.unit)}
                                        </span>
                                    </span>
                                </li>
                            ))}
